...
<body>
	...
	if(GENERATE_OUT) echo '<pre>', h($level0Generator->generate($level0)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level1Generator->generate($level1)), PHP_EOL, '</pre>';

	$level1to0Generator = new Level1\Generator('Level0');
	if(GENERATE_OUT) echo '<pre>', h($level1to0Generator->generate($level1to0)), PHP_EOL, '</pre>';

	$level1to2Generator = new Level1\Generator('Level2');
	if(GENERATE_OUT) echo '<pre>', h($level1to2Generator->generate($level1to2)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level2Generator->generate($level2)), PHP_EOL, '</pre>';

	$level2to1Generator = new Level2\Generator('Level1');
	if(GENERATE_OUT) echo '<pre>', h($level2to1Generator->generate($level2to1)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level3Generator->generate($level3)), PHP_EOL, '</pre>';

	$level3to2Generator = new Level3\Generator('Level2');
	if(GENERATE_OUT) echo '<pre>', h($level3to2Generator->generate($level3to2)), PHP_EOL, '</pre>';

	$level3to1Generator = new Level3\Generator('Level1');
	if(GENERATE_OUT) echo '<pre>', h($level3to1Generator->generate($level3to1)), PHP_EOL, '</pre>';

	$level3to0Generator = new Level3\Generator('Level0');
	if(GENERATE_OUT) echo '<pre>', h($level3to0Generator->generate($level3to0)), PHP_EOL, '</pre>';

	$level3to4Generator = new Level3\Generator('Level4');
	if(GENERATE_OUT) echo '<pre>', h($level3to4Generator->generate($level3to4)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level4Generator->generate($level4)), PHP_EOL, '</pre>';

	$level4to3Generator = new Level4\Generator('Level3');
	if(GENERATE_OUT) echo '<pre>', h($level4to3Generator->generate($level4to3)), PHP_EOL, '</pre>';

	$level4to2Generator = new Level4\Generator('Level2');
	if(GENERATE_OUT) echo '<pre>', h($level4to2Generator->generate($level4to2)), PHP_EOL, '</pre>';

	$level4to1Generator = new Level4\Generator('Level1');
	if(GENERATE_OUT) echo '<pre>', h($level4to1Generator->generate($level4to1)), PHP_EOL, '</pre>';

	$level4to0Generator = new Level4\Generator('Level0');
	if(GENERATE_OUT) echo '<pre>', h($level4to0Generator->generate($level4to0)), PHP_EOL, '</pre>';

	$level4to5Generator = new Level4\Generator('Level5');
	if(GENERATE_OUT) echo '<pre>', h($level4to5Generator->generate($level4to5)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level5Generator->generate($level5)), PHP_EOL, '</pre>';

	$level5to6Generator = new Level5\Generator('Level6');
	if(GENERATE_OUT) echo '<pre>', h($level5to6Generator->generate($level5to6)), PHP_EOL, '</pre>';

	$level5to4Generator = new Level5\Generator('Level4');
	if(GENERATE_OUT)  echo '<pre>', h($level5to4Generator->generate($level5to4)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level6Generator->generate($level6)), PHP_EOL, '</pre>';

	$level6to7Generator = new Level6\Generator('Level7');
	if(GENERATE_OUT) echo '<pre>', h($level6to7Generator->generate($level6to7)), PHP_EOL, '</pre>';

	$level6to5Generator = new Level6\Generator('Level5');
	if(GENERATE_OUT) echo '<pre>', h($level6to5Generator->generate($level6to5)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level7Generator->generate($level7)), PHP_EOL, '</pre>';

	$level7to8Generator = new Level7\Generator('Level8');
	if(GENERATE_OUT) echo '<pre>', h($level7to8Generator->generate($level7to8)), PHP_EOL, '</pre>';

	$level7to6Generator = new Level7\Generator('Level6');
	if(GENERATE_OUT) echo '<pre>', h($level7to6Generator->generate($level7to6)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level8Generator->generate($level8)), PHP_EOL, '</pre>';


	$level8to9Generator = new Level8\Generator('Level9');
	if(GENERATE_OUT) echo '<pre>', h($level8to9Generator->generate($level8to9)), PHP_EOL, '</pre>';

	$level8to7Generator = new Level8\Generator('Level7');
	if(GENERATE_OUT) echo '<pre>', h($level8to7Generator->generate($level8to7)), PHP_EOL, '</pre>';
	...
	if(GENERATE_OUT) echo '<pre>', h($level9Generator->generate($level9)), PHP_EOL, '</pre>';
	?>
</body>
</html>