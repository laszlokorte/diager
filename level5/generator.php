    ...
    public function generate(Diagram $diagram) {
        ...
        $relations = implode(PHP_EOL, array_map(function($relIndex, $rel) use($diagram) { 
            $params = implode(', ', [
                ...
                ...array_map(fn($att) => sprintf('public string $%s', $diagram->listOfRelationAttribute[$att->RelationAttributeIndex]->id), array_filter($diagram->listOfAttached, fn($att) => $att->RelationIndex === $relIndex))
            ]);
